...
一个 Obsidian 插件，通过右键菜单为选中文字快速添加、切换或清除颜色和字体大小，并能在粘贴 URL 时自动转换为 Markdown 链接。
...
- **粘贴即转链接** — 粘贴纯 HTTP/HTTPS URL 时，自动转换为 Markdown 链接 `[title](url)`
- **感知选区** — 粘贴时若已有选中文本，则立即将其作为链接文本（`[选中文本](url)`），无需网络请求
- **异步抓取标题** — 无选区时先插入占位符，随后在后台抓取页面标题并替换
- **失败优雅回退** — 标题抓取失败（超时、HTTP 错误、私有地址等）时，以 URL 本身作为链接文本（`[url](url)`）
- **隐私保护** — 可选跳过 localhost 与私有 IP 段（10.x、172.16–31.x、192.168.x、`.local` 等）的标题抓取
- **排除域名** — 逗号分隔的域名清单可让指定站点保留 Obsidian 原生粘贴行为，支持子域与通配符匹配（`example.com`、`.example.com`、`*.example.com`）
- **编码兼容** — 正确解码非 UTF-8 页面（含 GBK/GB2312）的标题
...
### 超链接
- **启用自动超链接** — 开关粘贴转链接行为
- **抓取超时（毫秒）** — 等待页面标题抓取的最长时间（范围 1000–60000）
- **跳过私有地址** — 不对 localhost 或私有 IP 地址发起标题抓取
- **User-Agent** — 抓取页面标题时使用的请求头
- **排除域名** — 不进行标题抓取的域名（逗号分隔），支持子域与通配符（`example.com`、`*.example.com`）
